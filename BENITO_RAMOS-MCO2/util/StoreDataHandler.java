...
public class StoreDataHandler {
    ...
    private static final String PRODUCT_SALES_FILE = "product_sales.csv";
    ...
    public StoreDataHandler() {
        ...
        try {
            ...
            new FileWriter(PRODUCT_SALES_FILE, true).close();
        } catch (IOException e) {
    ...
    /**
     * Saves product sales to file.
     * Format: saleID,targetID,discountType,discountValue,startDate,endDate,isActive
     */
    public void saveSales(SaleManager saleManager) {
        try (PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(PRODUCT_SALES_FILE)))) {
            for (ProductSale sale : saleManager.getAllSales()) {
                StringJoiner csvLine = new StringJoiner(",");
                csvLine.add(sale.getSaleID());
                csvLine.add(sale.getTargetID());
                csvLine.add(sale.getDiscountType());
                csvLine.add(String.valueOf(sale.getDiscountValue()));
                csvLine.add(sale.getStartDate().toString());
                csvLine.add(sale.getEndDate().toString());
                csvLine.add(String.valueOf(sale.isActive()));
                out.println(csvLine.toString());
            }
        } catch (IOException e) {
            System.err.println("Error saving product sales: " + e.getMessage());
        }
    }
    
    /**
     * Loads product sales from file.
     */
    public void loadSales(SaleManager saleManager) {
        try (BufferedReader br = new BufferedReader(new FileReader(PRODUCT_SALES_FILE))) {
            String line;
            while ((line = br.readLine()) != null) {
                if (line.trim().isEmpty()) continue;
                ...
                try {
                    ...
                    String discountType = data[2];
                    double discountValue = Double.parseDouble(data[3]);
                    java.time.LocalDateTime startDate = java.time.LocalDateTime.parse(data[4]);
                    java.time.LocalDateTime endDate = java.time.LocalDateTime.parse(data[5]);
                    boolean isActive = Boolean.parseBoolean(data[6]);
                    
                    // Create sale directly with existing ID
                    ProductSale sale = new ProductSale(saleID, targetID, discountType, discountValue, startDate, endDate);
                    sale.setActive(isActive);
                    saleManager.getAllSales().add(sale);
                    
                    // Update next sale ID counter in SaleManager
                    if (saleID.startsWith("SALE-")) {
                        int id = Integer.parseInt(saleID.substring(5));
                        saleManager.updateNextSaleID(id + 1);
                    }
                } catch (Exception e) {
