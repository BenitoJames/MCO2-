...
public class CustomerGUI extends JPanel {
    ...
    private JPanel createBottomPanel() {
        ...
        // Add transaction history button for non-guest customers
        if (!customer.isGuest()) {
            JButton historyButton = new JButton("My Transactions");
            historyButton.setFont(new Font("Arial", Font.BOLD, 14));
            historyButton.setBackground(new Color(63, 81, 181));
    ...
    /**
     * Shows transaction history for the signed-in customer.
     */
    private void showTransactionHistory() {
        try {
            java.io.BufferedReader reader = new java.io.BufferedReader(
                new java.io.FileReader("receipts.log")
            );
            
            StringBuilder history = new StringBuilder();
            String line;
            boolean foundTransactions = false;
            boolean inCustomerTransaction = false;
            StringBuilder currentTransaction = new StringBuilder();
            
            while ((line = reader.readLine()) != null) {
                // Check if this line starts a transaction for this customer
                if (line.contains("Customer: " + customer.getUserID()) || 
                    line.contains("Customer ID: " + customer.getUserID())) {
                    inCustomerTransaction = true;
                    foundTransactions = true;
                    currentTransaction = new StringBuilder();
                    currentTransaction.append(line).append("\n");
                } else if (inCustomerTransaction) {
                    currentTransaction.append(line).append("\n");
                    // Check for end of receipt marker
                    if (line.contains("====================") && 
                        currentTransaction.toString().contains("Total Amount")) {
                        history.append(currentTransaction).append("\n");
                        inCustomerTransaction = false;
                    }
                }
            }
            reader.close();
