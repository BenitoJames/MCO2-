        ...
        try {
            ...
            if (!checkoutComplete) {
                ...
                if (currentCustomer.hasMembership()) {
                    // This finds the transaction object if checkout started but failed
                    // In our current logic, points are only redeemed *during* handleCheckout,
                    // so we would need to pass the transaction object back...
                    // For simplicity, we assume if checkoutComplete is false, no points were used.
